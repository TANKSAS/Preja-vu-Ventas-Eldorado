...
public class MauricioAIController : ConvaiParametersEvaluator
{
    ...
    {
        Debug.Log("▶ Iniciando evaluación con Mauricio");

        // Activar UI y permisos
        ...
        // Enviar mensaje por primera vez
        SendPlayerMessage(dialogue);
        Debug.Log("Enviando respuesta: " + dialogue);

        // ---- 1. ESPERAR RESPUESTA DEL NPC O TIMEOUT ----
        yield return StartCoroutine(GameManager.Instance.chatController.WaitForNPCTalkingOrTimeout());

        // ---- 2. SI HAY TIMEOUT → MOSTRAR PANEL Y ESPERAR DECISIÓN ----
        if (GameManager.Instance.chatController.npcTimeoutOccurred)
        {
            Debug.Log("❌ Mauricio no respondió. Mostrando opciones al usuario...");

            // Mostrar panel de error y esperar Retry/Cancel
            ...
            {
                Debug.Log("🔁 Usuario eligió REINTENTAR. Reenviando mensaje...");

                SendPlayerMessage(dialogue);

                // Esperar nuevamente a que Mauricio responda
                yield return StartCoroutine(GameManager.Instance.chatController.WaitForNPCTalkingOrTimeout());

                // Si falla otra vez, se vuelve a mostrar el panel (comportamiento deseado)
                if (GameManager.Instance.chatController.npcTimeoutOccurred)
                {
                    Debug.LogError("❌ Mauricio volvió a fallar. Cancelando...");
                    yield break;
                }
            }
            ...
            {
                Debug.Log("🚫 Usuario canceló el ejercicio. Terminando flujo.");
                yield break;
            }
        }

        // ---- 3. SI LLEGAMOS AQUÍ → MAURICIO SÍ HABLÓ ----
        Debug.Log("📢 Mauricio está hablando, procesando resultados...");
