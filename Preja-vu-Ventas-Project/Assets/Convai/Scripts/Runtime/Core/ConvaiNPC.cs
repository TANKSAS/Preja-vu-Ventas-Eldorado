...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiNPC : MonoBehaviour
    {
        ...
        // Agrega esto en la sección de variables privadas
        public string _fullTranscript = "";
        public bool _isFinalResponseReceived;
        ...
        private void OnEnable()
        {
            ...
            if (GetComponent<ConvaiParametersEvaluator>())
            {
                AudioManager.OnAudioTranscriptAvailable += GetComponent<ConvaiParametersEvaluator>().GetNPCResponse;
                AudioManager.OnCharacterTalkingChanged += GetComponent<ConvaiParametersEvaluator>().IsNPCTalking;
        ...
        public void EnqueueResponse(GetResponseResponse response)
        {
            ...
            if (!string.IsNullOrEmpty(response.AudioResponse.TextData))
            {
                _fullTranscript += response.AudioResponse.TextData + " ";
            }

            // Marca si es la respuesta final
            if (response.AudioResponse.EndOfResponse)
            {
                _isFinalResponseReceived = true;
            }
