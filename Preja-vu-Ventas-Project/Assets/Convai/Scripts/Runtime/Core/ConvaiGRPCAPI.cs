...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiGRPCAPI : MonoBehaviour
    {
        ...
        private void ProcessAudioResponse(GetResponseResponse result, Queue<LipSyncBlendFrameData> lipSyncBlendFrameQueue, ref bool firstSilFound, ConvaiNPC npc)
        {
            ...
            if (result.AudioResponse?.EndOfResponse == true)
            {
                Debug.Log($"AudioResponse - EndOfResponse: {result.AudioResponse?.EndOfResponse} ");
                ConvaiNPCManager.Instance.activeConvaiNPC._isFinalResponseReceived = true;
                Debug.Log(" " + ConvaiNPCManager.Instance.activeConvaiNPC.characterName);
            }
